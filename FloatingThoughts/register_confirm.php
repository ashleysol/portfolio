...
if ( !isset($_POST['password']) || empty($_POST['password'])) {
    ...
    $stmt_reg = $mysqli->prepare("SELECT * FROM users WHERE user_name = ?");
	$stmt_reg->bind_param("s", $_POST["username"]);
	$exec_reg = $stmt_reg->execute();
	if(!$exec_reg) {
		echo $mysqli->error;
	}
	$stmt_reg->store_result();
	$numrows = $stmt_reg->num_rows;
	$stmt_reg->close();
